...
class GPT3_Naive_Baseline:
    ...
    def construct_naive_prompt(self, record, naive_template):
        if self.dataset_name == "LogicNLI":
            ...
            # Your original code: record['question'].split('?')[1] (or regex in construct_prompt_a)
            # Here we use the same behavior: take everything after the first '?'
    ...
    def save_output(self, outputs):
        ...
        file_name = f'{self.dataset_name}_{model_name}_naive_prompt.json'
    ...
    def process_example(self, example, naive_template):
        ...
        resp = self.openai_api.generate(prompt)
        ...
        explanation, final_answer = self.parse_naive_output(resp)
